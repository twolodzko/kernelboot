...
#' Draw samples from kernel density with Gaussian kernel
#'
...
#' @param method  algorithm to be used for sampling as described by Silverman (2000), \code{"default"}
#'                is the general algorithm and \code{"smoothed"} is the \emph{Step 3'} variant (p. 143).
...
rkde <- function(n, x, bw = "nrd0", method = c("default", "smoothed"),
  ...
  heps <- rnorm(n, 0, bw)
  switch (method,
    "smoothed" = mx + (x[idx]-mx+heps)/sqrt(1 + bw^2/sx),
   x[idx]+heps
  )
