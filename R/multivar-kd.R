...
#'
#' @param y         \eqn{n \times m}{n*m} numeric matrix.
...
#' univariate kernel \eqn{K} parametrized by bandwidth \eqn{h_j}{h[j]}, where
...
rmvk <- function(n, y, bw = sqrt(diag(bw.silv(y))),
  ...
  out <- cpp_rmvk(n, y, bw, weights, kernel, shrinked)$samples
