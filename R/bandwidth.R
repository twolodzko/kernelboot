...
#'
#' \deqn{
#' H = \left(n^{-1/(m+4)}\right)^2 \mathrm{diag}(S)
#' }{
#' H = [n^(-1/(m+4))]^2 * diag(S)
#' }
#'
...
#'
#' \deqn{
#' H = \left(\left(\frac{4}{(m+2)n}\right)^{1/(m+4)}\right)^2 \mathrm{diag}(S)
#' }{
#' H = [(4/((m+2)*n))^(1/(m+4))]^2 * diag(S)
#' }
#'
#' where \eqn{m} is number of variables, \eqn{n} is sampel size and \eqn{S}
